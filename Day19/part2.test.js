...
const { normalizeRules, buildRuleList } = require('./part2')
...
4: "a"
5: "b"`
        data = parseInput(inputa).reduce(normalizeRules, {})
        data1 = parseInput(input1).reduce(normalizeRules, {})
        data2 = parseInput(input).reduce(normalizeRules, {})
    })
    ...
    it('builds the new rule list', () => {
        const match = buildRuleList(data2, '31')
        console.log(match)
    })
})